...
**Impact**: ~12-18x speedup for service principal data collection - from 35 minutes to ~2-3 minutes
...
for sp in sp_batch:  # 4 SPs at a time
    # Use proper GUID format for request IDs
...
**Debugging Features**:
- Detailed logging of batch requests and responses
- Error messages include operation type and API error details
- Logs first request in each batch for verification

**Fallback**: If batch request fails, falls back to parallelized per-SP approach (5 parallel calls per SP with ThreadPoolExecutor).
...
- Service principal data collection (batch progress updates)
