...
The OID-See Viewer supports two methods for loading tenant data:
...
1. Click **Upload JSON** in the header (or drag-and-drop a file onto the main panel)
2. Select your OID-See export JSON file (generated by `oidsee_scanner.py`)
3. The file size is displayed in the loading overlay while the file is parsed
4. The view updates automatically once loading completes
...
**Desktop (> 1100px)**:
- Two-panel layout: Graph/View | Details
...
You can export the current graph data by copying nodes from the Table View or using the export actions in the details panel.
...
**Symptoms**: Switching to Graph tab shows no data or an error
...
- **React 19**: UI framework with `startTransition` for non-urgent state updates
- **TypeScript**: Type-safe development
- **vis-network**: Graph visualization library
- **Vite**: Build tool and dev server
- **Web Worker** (`src/workers/dataWorker.ts`): Offloads JSON parsing, filtering, and graph conversion from the main thread — the UI never blocks during import
...
Graph View is capped at **3,000 highest-risk nodes** and **4,500 edges** to maintain browser performance. A warning dialog is shown when your dataset exceeds these thresholds. Table, Tree, Matrix, and Dashboard views show the **full dataset** regardless of size (30k+ nodes supported).
