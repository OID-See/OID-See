...
def delta_to_csv_rows(delta: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    for entry in delta:
        rows.append({
            ...
            "previousRiskScore": str(entry["previousRiskScore"]) if entry.get("previousRiskScore") is not None else "",
            "currentRiskScore": str(entry["currentRiskScore"]) if entry.get("currentRiskScore") is not None else "",
